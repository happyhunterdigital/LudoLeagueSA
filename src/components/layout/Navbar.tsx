...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  // Removed 'Shop' from text links list to highlight it as an action button next to the cart
      ...
      <nav className="fixed top-0 left-0 right-0 z-[1000] px-6 md:px-12 py-4 transition-all duration-500 bg-[#2C3E50] shadow-lg">
        <div className="max-w-[1400px] mx-auto flex items-center justify-between gap-6">
          ...
          <div className="hidden lg:flex items-center gap-6 xl:gap-8">
            ...
            >
              <button 
                ...
                className={`text-[10px] xl:text-[11px] uppercase tracking-widest font-black italic transition-colors flex items-center gap-1 ${activeSection === 'about' || activeSection === 'faqs' ? 'text-[#FFD700]' : 'text-slate-300 hover:text-white'}`}
              >
            ...
            {navItems.slice(1).map((item) => (
              <button
                ...
                className={`text-[10px] xl:text-[11px] uppercase tracking-widest font-black italic transition-colors ${activeSection === item.toLowerCase() ? 'text-[#FFD700]' : 'text-slate-300 hover:text-white'}`}
              >
                {item}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2 sm:gap-4 shrink-0">
            ...
            {/* Shop Button highlighted in RED next to the cart */}
