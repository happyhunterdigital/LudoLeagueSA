...
export const Navbar: React.FC<NavbarProps> = ({ 
  ...
  return (
    <>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-[1000] px-4 md:px-10 py-4 md:py-6 border-b transition-all duration-500 bg-bg-panel/95 backdrop-blur-xl ${activeSection !== 'home' ? 'border-teal-400/20 py-3 shadow-lg' : 'border-teal-400/10'}`}>
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navItems.map((item) => (
              <button 
                ...
                className={`text-[11px] uppercase tracking-[0.25rem] font-black italic transition-all ${activeSection === item ? 'text-accent-teal' : 'text-white/50 hover:text-white'}`}
              >
                {item}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2 sm:gap-6">
            <div className="flex items-center gap-2 pr-2 md:pr-4 border-r border-white/10">
              <button onClick={() => scrollToSection('shop')} className="relative p-2 text-white/60 hover:text-red-500 transition-colors">
                <Heart size={18} fill={wishlist.length > 0 ? "currentColor" : "none"} className={wishlist.length > 0 ? "text-red-500" : ""} />
              </button>
              <button onClick={() => scrollToSection('shop')} className="relative p-2 text-white/60 hover:text-accent-teal transition-colors">
            ...
            <button onClick={() => scrollToSection('shop')} className="hidden sm:flex items-center gap-2 px-6 py-2 bg-accent-teal text-white hover:bg-white hover:text-accent-teal transition-all uppercase text-[10px] tracking-widest font-black italic shadow-lg">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-bg-panel border-b border-teal-400/20 py-4 px-4 flex flex-col gap-4 shadow-2xl">
            {navItems.map((item) => (
              <button key={item} onClick={() => scrollToSection(item)} className={`text-left p-3 text-sm uppercase tracking-widest font-black italic rounded-lg ${activeSection === item ? 'bg-accent-teal/10 text-accent-teal' : 'text-white/70 hover:bg-white/5'}`}>
                {item}
              </button>
            ))}
          </div>
