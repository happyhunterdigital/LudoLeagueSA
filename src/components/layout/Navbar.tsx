          ...
          <div className="flex items-center gap-2 sm:gap-6">
            <div className="flex items-center gap-2 pr-2 md:pr-4 border-r border-slate-600">
              ...
              <button onClick={() => scrollToSection('shop')} className="relative p-2 text-slate-300 hover:text-[#FFD700] transition-colors">
                ...
                {cart.length > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 bg-[#FFD700] text-[#2C3E50] text-[9px] font-bold flex items-center justify-center rounded-full">
                    {cart.length}
                  </span>
                )}
              </button>
          ...
          <div className="lg:hidden absolute top-full left-0 right-0 bg-[#2C3E50] border-t border-slate-700 py-4 px-4 flex flex-col gap-4 shadow-2xl">
            ...
            {navItems.slice(1).map((item) => (
              <button
                key={item}
                onClick={() => handleNavClick(item)}
                className={`text-left p-3 text-sm uppercase tracking-widest font-black italic rounded-xl ${activeSection === item.toLowerCase() ? 'bg-slate-800 text-[#FFD700]' : 'text-slate-300 hover:bg-slate-800'}`}
              >
                {item}
              </button>
            ))}
