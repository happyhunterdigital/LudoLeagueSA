              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {paymentMethod === 'eft' ? (
                  <div className="space-y-6">
                    <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-2 text-sm text-slate-700">
                      ...
                      <p className="pt-2 border-t text-base text-[#001F3F]"><b>Grand Total:</b> <span className="text-[#0EA5E9] font-black">R{grandTotal.toLocaleString()}</span></p>
                    </div>
                ...
                <div className="flex gap-4">
                  ...
                  <button type="submit" disabled={isSubmitting || (paymentMethod === 'eft' && !formData.proofOfPayment)} className="w-1/2 py-4 bg-[#D32F2F] hover:bg-slate-900 text-white font-black uppercase tracking-widest rounded-xl disabled:opacity-50 transition-all flex items-center justify-center shadow-lg">
                    {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : paymentMethod === 'payfast' ? 'Pay Now' : 'Place Order'}
                  </button>
                </div>
              </form>
            )}
