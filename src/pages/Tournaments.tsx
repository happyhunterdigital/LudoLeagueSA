          ...
          <form onSubmit={handleRegister} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              <div>
                ...
                <input type="text" required maxLength={100} className="w-full bg-white border rounded-xl p-4 outline-none transition-colors shadow-sm" style={{ color: 'var(--text)', borderColor: 'var(--border-color)' }} value={formData.fullName} onChange={e => setFormData({...formData, fullName: e.target.value})} />
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest mb-2 opacity-80">Email</label>
                <input type="email" required maxLength={128} className="w-full bg-white border rounded-xl p-4 outline-none transition-colors shadow-sm" style={{ color: 'var(--text)', borderColor: 'var(--border-color)' }} value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest mb-2 opacity-80">Phone</label>
                <input type="tel" required maxLength={20} className="w-full bg-white border rounded-xl p-4 outline-none transition-colors shadow-sm" style={{ color: 'var(--text)', borderColor: 'var(--border-color)' }} value={formData.phoneNumber} onChange={e => setFormData({...formData, phoneNumber: e.target.value})} />
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest mb-2 opacity-80">Region</label>
                <select className="w-full bg-white border rounded-xl p-4 outline-none appearance-none transition-colors shadow-sm" style={{ color: 'var(--text)', borderColor: 'var(--border-color)' }} value={formData.region} onChange={e => setFormData({...formData, region: e.target.value as any})}>
