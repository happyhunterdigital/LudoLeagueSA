      ...
      <section id="leagues" className="min-h-screen w-full relative flex flex-col justify-center py-24 px-4 md:px-10 bg-[#0EA5E9] text-[#0F172A] border-b border-white/15">
        <div className="max-w-7xl mx-auto w-full flex flex-col md:flex-row items-center gap-12">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="md:w-1/2 space-y-6">
            ...
            <p className="text-slate-800 leading-relaxed mb-6">South Africa's fastest-growing competitive district. High strategy, dedicated fanbases, and weekly neighborhood fixtures that lock out local community halls.</p>
            <button onClick={() => handleLaunchGallery('mamelodi')} className="btn-action bg-[#0F172A] text-white">Launch Mamelodi Gallery &rarr;</button>
          </motion.div>
      ...
      <section className="min-h-screen w-full relative flex flex-col justify-center py-24 px-4 md:px-10 bg-[#0F172A] text-white border-b border-white/15">
        <div className="max-w-7xl mx-auto w-full flex flex-col-reverse md:flex-row items-center gap-12">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="md:w-1/2 space-y-6">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">The ultimate township showdown. What could be more exciting than a clash between two regional powerhouses of Ludo, where players from both communities go head-to-head in a high-stakes, winner-takes-all knockout tournament? Rivalries are ignited, champions are crowned, and legends are born. This event showcases the growth, passion, and impact of the sport on a scale never witnessed before.</p>
            <button onClick={() => handleLaunchGallery('botk')} className="btn-action bg-[#FFC107] text-[#0F172A] font-black uppercase tracking-widest">Launch BOTK Gallery &rarr;</button>
          </motion.div>
        </div>
      </section>
      ...
      <section className="min-h-screen w-full relative flex flex-col justify-center py-24 px-4 md:px-10 bg-[#0EA5E9] text-[#0F172A]">
        <div className="max-w-7xl mx-auto w-full flex flex-col md:flex-row items-center gap-12">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="md:w-1/2 space-y-6">
            ...
            <p className="text-slate-700 font-bold leading-relaxed text-lg">District HQ • Est. 2022 • Soweto, Gauteng</p>
            <p className="text-slate-800 leading-relaxed mb-6">A league hosted in one of the largest and most iconic townships in the country, a place rich in history, culture, and significance. And it certainly did not disappoint. Ludo found a natural home among the people of Soweto.</p>
            <button onClick={() => handleLaunchGallery('soweto')} className="btn-action bg-[#0F172A] text-white">Launch Soweto Gallery &rarr;</button>
          </motion.div>
